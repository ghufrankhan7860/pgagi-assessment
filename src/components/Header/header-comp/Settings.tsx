...
const MoviesSettings = () => {
    ...
    const selectedmQueries = useSelector(
        (store: RootState) => store.preferences.mQueries
    );

    const handleGenre = (genres: string) => {
        if (selectedGenres.includes(genres) === false) {
            const newGenres = [...selectedGenres, genres];
...
const Settings = () => {
    ...
    return (
        <>
            {currentPath === "/feed" ||
            currentPath === "/feed/news" ||
            currentPath === "/trending" ||
            currentPath === "/trending/news" ? (
                <NewsSettings />
            ) : (
                <MoviesSettings />
            )}
